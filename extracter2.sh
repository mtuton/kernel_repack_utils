...
##
# For initramfs's modified by z4build (z4mod) some initramfs files are brute force stuck in a tar at the 
# end of the zImage and unpacked early in the init script. To unpack the whole thing you need to do 
# something like (basically taken from the z4build init wrapper script itself):
##
#offset="`dd if=$zImagefile bs=4 skip=11 count=1 2>/dev/null | od -l|tr -s " "|head -n 1|cut -f2 -d " "`"
#offset=`echo $offset | cut -f 2 -d" "`
#dd if=$zImagefile bs=$offset skip=1 | tar xz
#normal_extraction_script.sh $zImagefile
